...
try:
    ...
    # Manual .env parsing — no external dependency needed
...
DB_PATH      = os.getenv('DB_PATH',      'phishguard.db')
SECRET_KEY   = os.getenv('SECRET_KEY',   'phishguard-secret-key-change-in-production')
FRONTEND_URL = os.getenv('FRONTEND_URL', 'http://127.0.0.1:8088')
...
def init_db():
    ...
    admin_password = hash_password(os.getenv('ADMIN_PASSWORD', 'admin123'))
    c.execute('''
        INSERT OR IGNORE INTO users (name, email, password, role, department)
        VALUES (?, ?, ?, ?, ?)
    ''', (os.getenv('ADMIN_NAME','Super Admin'), os.getenv('ADMIN_EMAIL','admin@phishguard.com'), admin_password, 'admin', 'IT Security'))
...
def test_smtp_profile(pid):
    ...
    from_name  = (p.get('from_name')  or 'PhishGuard').strip()

    if not from_email or '@' not in from_email:
        return jsonify({'success': False, 'error': f'Profile has no valid From Email. Edit it first.'}), 400

    import time as _t
    # Always unique name — never conflicts, no delete needed
    test_name = f'PG_TEST_SMTP_{pid}_{int(_t.time())}'
    try:
        result = gophish_request('smtp', 'POST', {
            'name':               test_name,
            'host':               f'{p["host"]}:{p["port"]}',
            'from_address':       f'{from_name} <{from_email}>',
            'username':           p.get('username') or '',
            'password':           p.get('password') or '',
            'ignore_cert_errors': bool(p.get('ignore_cert_errors', 1)),
        })
...
# ── Direct SMTP Test (bypasses GoPhish) ───────────────────────────────────
@app.route('/api/smtp-profiles/<int:pid>/send-test-email', methods=['POST'])
def send_test_email_direct(pid):
    """Send a real test email directly via smtplib — bypasses GoPhish entirely."""
    import smtplib
    ...
    conn = get_db()
    p = conn.execute('SELECT * FROM smtp_profiles WHERE id=?', (pid,)).fetchone()
    conn.close()
    if not p:
        return jsonify({'error': 'Profile not found'}), 404
    p = dict(p)
    ...
    from_name  = (p.get('from_name')  or 'PhishGuard').strip()
    if not from_email or '@' not in from_email:
        return jsonify({'error': 'Profile has no valid From Email'}), 400

    try:
        msg = MIMEMultipart('alternative')
        msg['Subject'] = 'PhishGuard — SMTP Test'
        msg['From']    = f'{from_name} <{from_email}>'
        msg['To']      = to_email
        html_body = '<html><body><h2>PhishGuard SMTP Test</h2><p>If you see this, SMTP is working!</p></body></html>'
        ...
        if p.get('use_ssl'):
            ...
            with smtplib.SMTP_SSL(host, port, context=ctx) as s:
                if p.get('username'): s.login(p['username'], p.get('password',''))
...
<body style="font-family:Arial,sans-serif;text-align:center;padding:60px 20px;background:#020b18;color:#cde4f8">
  <div style="max-width:500px;margin:0 auto;background:#071628;border:1px solid #0d2847;border-radius:16px;padding:40px">
    ...
    <a href="" + FRONTEND_URL + "/awareness-training.html"
...
def send_campaign_emails(campaign_id):
    ...
    if not targets:
        ...
        targets = [dict(r) for r in fallback]
    ...
    try:
        ...
        if not _active:
            _active = _conn.execute("SELECT * FROM smtp_profiles ORDER BY id LIMIT 1").fetchone()
        _conn.close()
        if not _active:
            return jsonify({"error": "No SMTP profile — add one in Phish Profiles."}), 500
        _p = dict(_active)
        _from_email = (_p.get("from_email") or "").strip()
        _from_name  = (_p.get("from_name")  or "PhishGuard").strip()
        if not _from_email or "@" not in _from_email:
            return jsonify({"error": "Active SMTP profile has no valid From Email. Edit it in Phish Profiles."}), 400
        print(f"\n📧 SMTP: {_p['name']} | {_p['host']}:{_p['port']} | {_from_email}")

        # ── 1. Load template ───────────────────────────────────────────────
        template_name = (campaign["template"] or "").strip() or "IT Password Reset"
        try:
            template = get_template_by_name(template_name)
        except Exception:
            all_t = load_templates()
            template = all_t[0] if all_t else {}
            print(f"⚠️  Template not found, fallback: {template.get('name','?')}")

        # ── 2. Resolve sender ──────────────────────────────────────────────
        sender_name  = (campaign["sender_name"]  or "").strip() or template.get("sender_name", "") or _from_name
        sender_email = (campaign["sender_email"] or "").strip() or template.get("sender_email", "") or _from_email
        if "@" not in sender_email:
            sender_email = _from_email
        print(f"📧 Sender: {sender_name} <{sender_email}>")

        # ── 3. Unique names ────────────────────────────────────────────────
        gp_smtp_name  = f"PG_SMTP_{_p['id']}_{_ts}"
        gp_tmpl_name  = f"PG_Tmpl_{campaign_id}_{_ts}"
        gp_page_name  = f"PG_Page_{campaign_id}_{_ts}"
        gp_group_name = f"PG_Group_{campaign_id}_{_ts}"
        gp_camp_name  = f"PG_Camp_{campaign_id}_{_ts}"

        # ── Step 1: Push SMTP ──────────────────────────────────────────────
        gophish_request("smtp", "POST", {
            "name":               gp_smtp_name,
            "host":               f"{_p['host']}:{_p['port']}",
            "from_address":       f"{_from_name} <{_from_email}>",
            "username":           _p.get("username") or "",
            "password":           _p.get("password") or "",
            "ignore_cert_errors": bool(_p.get("ignore_cert_errors", 1)),
        })
        print(f"✅ SMTP pushed: {gp_smtp_name}")

        # ── Step 2: Push email template ───────────────────────────────────
        gophish_request("templates", "POST", {
            "name":            gp_tmpl_name,
            "subject":         template.get("subject", "Important Notice"),
            "html":            template.get("html", "<p>Test email</p>"),
            "envelope_sender": f"{sender_name} <{sender_email}>",
        })
        print(f"✅ Template pushed: {gp_tmpl_name}")

        # ── Step 3: Push landing page ─────────────────────────────────────
        landing_html = AWARENESS_PAGE_HTML
        landing_file = template.get("landing_file", "")
        if landing_file:
            try:
                with open(os.path.join(TEMPLATES_DIR, landing_file)) as lf:
                    landing_html = lf.read()
                print(f"✅ Landing loaded: {landing_file}")
            except Exception as le:
                print(f"⚠️  Could not load landing {landing_file}: {le}")
        gophish_request("pages", "POST", {
            "name":                gp_page_name,
            "html":                landing_html,
            "capture_credentials": True,
            "capture_passwords":   False,
            "redirect_url":        FRONTEND_URL + "/awareness-training.html?caught=1&campaign_id=" + str(campaign_id),
        })
        print(f"✅ Landing page pushed: {gp_page_name}")

        # ── Step 4: Push targets group ────────────────────────────────────
        gophish_request("groups", "POST", {
            "name": gp_group_name,
            "targets": [{
                "first_name": (t["name"] or "").split(" ")[0] or "Employee",
                "last_name":  " ".join((t["name"] or "").split(" ")[1:]),
                "email":      t["email"],
                "position":   t["department"] if t["department"] else "",
            } for t in targets]
        })
        print(f"✅ Group pushed: {len(targets)} targets")

        # ── Step 5: Launch GoPhish campaign ───────────────────────────────
        result = gophish_request("campaigns", "POST", {
            "name":        gp_camp_name,
            "template":    {"name": gp_tmpl_name},
            "page":        {"name": gp_page_name},
            "smtp":        {"name": gp_smtp_name},
            "url":         BACKEND_URL + "/api/track/click?campaign_id=" + str(campaign_id) + "&email={{.Email}}",
            "launch_date": (datetime.utcnow() - timedelta(minutes=2)).strftime("%Y-%m-%dT%H:%M:%S+00:00"),
            "groups":      [{"name": gp_group_name}],
        })
        print(f"✅ GoPhish campaign launched. ID: {result['id']}")

        # ── Step 6: Save email logs to DB ─────────────────────────────────
        conn = get_db()
        conn.execute("UPDATE campaigns SET status='active' WHERE id=?", (campaign_id,))
        raw_html  = template.get("html", "")
        click_base   = BACKEND_URL + "/api/track/click?campaign_id=" + str(campaign_id)
        report_base  = BACKEND_URL + "/api/track/report-link?campaign_id=" + str(campaign_id)
        open_base    = BACKEND_URL + "/api/track/open?campaign_id=" + str(campaign_id)

        for t in targets:
            first_name   = (t["name"] or "").split(" ")[0] or "Employee"
            t_email = t["email"]
            t_name  = t["name"] or ""
            t_dept  = t["department"] if t["department"] else ""
            click_url     = click_base  + "&email=" + t_email
            report_url    = report_base + "&email=" + t_email
            open_url      = open_base   + "&email=" + t_email
            open_pixel    = f'<img src="{open_url}" width="1" height="1" style="display:none"/>'
            report_footer = (f'<div style="margin-top:24px;font-size:11px;color:#aaa;text-align:center">' +
                             f'<a href="{report_url}" style="color:#aaa">Unsubscribe</a></div>')
            personalised = raw_html
            for ph, val in [("{{.FirstName}}", first_name),
                             ("{{.Email}}",     t_email),
                             ("{{.URL}}",       click_url),
                             ("{{.Tracker}}",   open_pixel)]:
                personalised = personalised.replace(ph, val)
            if open_pixel not in personalised:
                personalised += open_pixel
            personalised += report_footer
            conn.execute(
                "INSERT INTO email_logs (campaign_id,recipient_email,recipient_name,subject,body_html,sent_at,status) VALUES (?,?,?,?,?,datetime('now'),'sent')",
                (campaign_id, t_email, t_name, template.get("subject", ""), personalised)
            )
        conn.commit()
        conn.close()
        print(f"✅ {len(targets)} email logs saved")

        return jsonify({
            "success":    True,
            "gophish_id": result["id"],
            "targets":    len(targets),
            "message":    f"Campaign launched — {len(targets)} emails queued ({_p['name']})"
        })
    except Exception as e:
        import traceback
...
def track_report_link():
    ...
    return redirect(FRONTEND_URL + '/unsubscribe.html')
...
def migrate_db():
    ...
    if conn.execute('SELECT COUNT(*) FROM smtp_profiles').fetchone()[0] == 0:
        conn.execute('''
            ...
            VALUES ('Mailhog (Local)', 'mailhog', '127.0.0.1', 1025, 'PhishGuard', 'phishguard@phishguard.local', 1)
        ''')
    ...
    # Fix Mailhog profiles with invalid from_email
    conn.execute("""
        UPDATE smtp_profiles
        SET from_email = 'phishguard@phishguard.local'
        WHERE (from_email = 'phishguard@localhost' OR from_email = '' OR from_email IS NULL)
          AND type = 'mailhog'
    """)
